...
def make_vec(image_x = [], image_y = [], test_num = 5):
    ...
    vec_1 = []
    for x in image_x:
        vec_0.append(model(np.zeros((3,3,224,224), dtype=np.float32))["prob"].data)
    for x in image_y:
        vec_1.append(model(np.zeros((3,3,224,224), dtype=np.float32))["prob"].data)
